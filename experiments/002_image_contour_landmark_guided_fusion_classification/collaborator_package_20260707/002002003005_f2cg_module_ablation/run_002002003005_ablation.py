...
CHECKPOINT_ROOT = MODEL_ROOT / "Generated_Models" / "002002003005_f2cg_module_ablation"
...
OUTPUT_DIR = OUTPUT_ROOT / "unconfigured"
CHECKPOINT_DIR = CHECKPOINT_ROOT / "unconfigured"
...
ABLATION_VARIANTS = {
    "002002003005001_gated_only": {
        "label": "gated-only F2CG",
        "dir_name": "001_gated_only",
        "use_gated_fusion": True,
        "use_fractal_embedding": False,
        "use_fractional_embedding": False,
        "use_fractal_loss": False,
    },
    "002002003005002_fractal_only": {
        "label": "fractal-only F2CG",
        "dir_name": "002_fractal_only",
        "use_gated_fusion": False,
        "use_fractal_embedding": True,
        "use_fractional_embedding": False,
        "use_fractal_loss": True,
    },
    "002002003005003_fractional_only": {
        "label": "fractional-only F2CG",
        "dir_name": "003_fractional_only",
        "use_gated_fusion": False,
        "use_fractal_embedding": False,
        "use_fractional_embedding": True,
        "use_fractal_loss": True,
    },
    "002002003005004_gated_fractal": {
        "label": "gated + fractal F2CG",
        "dir_name": "004_gated_fractal",
        "use_gated_fusion": True,
        "use_fractal_embedding": True,
        "use_fractional_embedding": False,
        "use_fractal_loss": True,
    ...
    "002002003005005_gated_fractional": {
        "label": "gated + fractional F2CG",
        "dir_name": "005_gated_fractional",
        "use_gated_fusion": True,
        "use_fractal_embedding": False,
        "use_fractional_embedding": True,
        "use_fractal_loss": True,
    ...
    "002002003005006_fractal_fractional_no_gates": {
        "label": "fractal + fractional F2CG without gates",
        "dir_name": "006_ff_no_gates",
        "use_gated_fusion": False,
        "use_fractal_embedding": True,
        "use_fractional_embedding": True,
        "use_fractal_loss": True,
...
def configure_variant_paths(variant: str) -> None:
    global OUTPUT_DIR, CHECKPOINT_DIR, TARGET_PATH, METADATA_PATH, FOLDS_PATH, OUTER_SPLITS_PATH
    dir_name = ABLATION_VARIANTS[variant]["dir_name"]
    OUTPUT_DIR = OUTPUT_ROOT / dir_name
    CHECKPOINT_DIR = CHECKPOINT_ROOT / dir_name
    TARGET_PATH = OUTPUT_DIR / TARGET_NAME
    METADATA_PATH = OUTPUT_DIR / "metadata.csv"
    FOLDS_PATH = OUTPUT_DIR / "folds.csv"
    OUTER_SPLITS_PATH = OUTPUT_DIR / "outer_splits.csv"
...
class RunConfig:
    ...
    use_fractal_embedding: bool
    use_fractional_embedding: bool
    use_fractal_loss: bool
    seed: int
    n_splits: int
    image_size: int
    batch_size: int
...
class F2CGNet(nn.Module):
    def __init__(
        ...
        self.use_fractional_embedding = use_fractional_embedding
        ...
        final_dim = 256
        if self.use_fractal_embedding:
            final_dim += fractal_embedding_dim
        if self.use_fractional_embedding:
            final_dim += fractional_embedding_dim
    ...
    def forward(self, x):
        ...
        d4 = self.dec4(c5)
        f4 = self.gate4(c4, d4) if self.use_gated_fusion else c4
        d3 = self.dec3(d4)
        f3 = self.gate3(c3, d3) if self.use_gated_fusion else c3
        d2 = self.dec2(d3)
        f2 = self.gate2(c2, d2) if self.use_gated_fusion else c2
        d1 = self.dec1(d2)
        d0 = self.dec0(d1)
        ...
        fused_parts = [self.image_projection(image_feature)]
        if self.use_fractal_embedding:
            fused_parts.append(fractal_out["embedding"])
        if self.use_fractional_embedding:
            fused_parts.append(fractional_out["embedding"])
...
def build_model(config: RunConfig, allow_random_fallback: bool) -> F2CGNet:
    return F2CGNet(
        ...
        use_fractal_embedding=config.use_fractal_embedding,
        use_fractional_embedding=config.use_fractional_embedding,
...
def compute_losses(model, images, labels, masks, cls_criterion, contour_bce, config: RunConfig):
    ...
    if config.use_fractal_loss:
        ...
        fractal_loss = cls_loss.new_tensor(0.0)
...
def train_fold(folds, outer_splits, masks_by_row_id, fold: int, args, config: RunConfig):
    ...
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    ckpt_path = CHECKPOINT_DIR / f"{config.variant}_fold{fold}.pt"
    torch.save(
        {
...
def write_summary(config: RunConfig, folds: pd.DataFrame, trained: bool, input_status=None, metrics=None) -> None:
    ...
    lines = [
        "# 002002003005 Module Ablation Run Summary",
        "",
        "## Status",
        "",
        f"- Variant: `{config.variant}`",
        f"- Variant label: {config.variant_label}",
        "- Model family: F2CG-Net module ablation",
        "- Direct baselines: 002002002 contour auxiliary, 002002003001 direct contour fusion, and 002002003002 full F2CG-Net",
        ...
        f"- Gated fusion: {config.use_gated_fusion}",
        f"- Fractal embedding: {config.use_fractal_embedding}",
        f"- Fractional embedding: {config.use_fractional_embedding}",
        f"- Fractal consistency loss: {config.use_fractal_loss}",
        f"- Samples: {len(folds)}",
        ...
        json.dumps(
            {
                ...
                "use_fractal_embedding": config.use_fractal_embedding,
                "use_fractional_embedding": config.use_fractional_embedding,
                "use_fractal_loss": config.use_fractal_loss,
...
def main() -> None:
    ...
    parser.add_argument("--variant", choices=sorted(ABLATION_VARIANTS), default="002002003005001_gated_only")
    ...
    variant_info = ABLATION_VARIANTS[args.variant]
    configure_variant_paths(args.variant)

    config = RunConfig(
        variant=args.variant,
        variant_label=variant_info["label"],
        use_gated_fusion=variant_info["use_gated_fusion"],
        use_fractal_embedding=variant_info["use_fractal_embedding"],
        use_fractional_embedding=variant_info["use_fractional_embedding"],
        use_fractal_loss=variant_info["use_fractal_loss"],
        seed=args.seed,
    ...
    print(f"Loaded {len(folds)} 002002003005 samples for {config.variant} from {OUTPUT_DIR}")
