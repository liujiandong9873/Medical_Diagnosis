...
CHECKPOINT_DIR = MODEL_ROOT / "Generated_Models" / "002002003001_contour_fusion_resnet18"
...
class XrayContourFusionDataset(Dataset):
    def __init__(
        self,
        frame: pd.DataFrame,
...
def contour_feature_block(in_channels: int, out_channels: int) -> nn.Sequential:
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=False),
        nn.BatchNorm2d(out_channels),
        nn.ReLU(inplace=True),
...
class ResNet18ContourFusion(nn.Module):
    def __init__(
        self,
        pretrained: bool,
        contour_feature_dim: int,
        ...
        self.contour_encoder = nn.Sequential(
            contour_feature_block(2, 16),
            contour_feature_block(16, 32),
            contour_feature_block(32, 64),
            contour_feature_block(64, contour_feature_dim),
        ...
        self.contour_projection = nn.Sequential(
            nn.Linear(contour_feature_dim, contour_feature_dim),
            nn.LayerNorm(contour_feature_dim),
            nn.ReLU(inplace=True),
        )
        self.fusion_classifier = nn.Sequential(
            nn.Linear(256 + contour_feature_dim, fusion_hidden_dim),
    ...
    def forward(self, x):
        ...
        image_feature = torch.flatten(self.avgpool(features), 1)
        contour_logits = self.decoder(features)
        contour_probs = torch.sigmoid(contour_logits)
        contour_feature = self.contour_encoder(contour_probs)
        fused = torch.cat(
            [
                self.image_projection(image_feature),
                self.contour_projection(contour_feature),
            ],
            dim=1,
        )
        cls_logits = self.fusion_classifier(fused).squeeze(1)
        return cls_logits, contour_logits, contour_feature
...
def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[int, np.ndarray]]:
    ...
    for path in [METADATA_PATH, FOLDS_PATH, OUTER_SPLITS_PATH, TARGET_PATH]:
        if not path.exists():
            raise FileNotFoundError(f"Missing required 002002003001 input: {path}")
...
def make_loaders(train_frame, val_frame, test_frame, masks_by_row_id, config: RunConfig, device):
    train_set = XrayContourFusionDataset(train_frame, masks_by_row_id, config.image_size, training=True)
    val_set = XrayContourFusionDataset(val_frame, masks_by_row_id, config.image_size, training=False)
    test_set = XrayContourFusionDataset(test_frame, masks_by_row_id, config.image_size, training=False)
    kwargs = {
...
def build_model(config: RunConfig, allow_random_fallback: bool) -> ResNet18ContourFusion:
    return ResNet18ContourFusion(
        pretrained=config.pretrained,
        contour_feature_dim=config.contour_feature_dim,
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(training):
            with amp_autocast(device, config.amp):
                cls_logits, contour_logits, _ = model(images)
...
def evaluate_with_ids(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            masks = batch["mask"].to(device, non_blocking=True)
            with amp_autocast(device, config.amp):
                cls_logits, contour_logits, contour_features = model(images)
                cls_loss = cls_criterion(cls_logits, labels)
                c_loss, c_bce, c_dice_loss = contour_loss_fn(contour_logits, masks, contour_bce)
                loss = cls_loss + config.lambda_contour * c_loss
            probs = torch.sigmoid(cls_logits).detach().cpu().numpy()
            labels_np = labels.detach().cpu().numpy().astype(int)
            contour_norms = torch.linalg.vector_norm(contour_features.detach(), dim=1).cpu().numpy()
            ...
            contour_logits_all.append(contour_logits.detach().cpu())
            contour_targets_all.append(masks.detach().cpu())
            for row_id, sample_id, image_file, fold, label, prob, c_norm in zip(
                batch["row_id"],
                batch["sample_id"],
                batch["image_file"],
                batch["fold"],
                ...
                records.append(
                    {
                        ...
                        "contour_feature_norm": float(c_norm),
                    }
                )
...
def train_fold(folds, outer_splits, masks_by_row_id, fold: int, args, config: RunConfig):
    ...
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    ckpt_path = CHECKPOINT_DIR / f"contour_fusion_resnet18_fold{fold}.pt"
    torch.save(
        {
...
def write_summary(config: RunConfig, folds: pd.DataFrame, trained: bool, input_status=None, metrics=None) -> None:
    ...
    lines = [
        "# 002002003001 Run Summary",
        "",
        "## Status",
        "",
        "- Model: ResNet18 shared encoder + contour decoder + predicted contour feature encoder + fusion classifier",
        "- Direct baselines: 002002001 matched image-only and 002002002 contour auxiliary supervision",
...
def run_smoke_test(folds, outer_splits, masks_by_row_id, args, config: RunConfig) -> None:
    ...
    loader = DataLoader(
        XrayContourFusionDataset(smoke_frame, masks_by_row_id, config.image_size, training=True),
        ...
        json.dumps(
            {
                ...
                "contour_features_shape": list(contour_features.shape),
...
def main() -> None:
    ...
    parser.add_argument("--contour-feature-dim", type=int, default=128)
    ...
    config = RunConfig(
        ...
        contour_feature_dim=args.contour_feature_dim,
    ...
    print(f"Loaded {len(folds)} 002002003001 samples from {OUTPUT_DIR}")
