...
CHECKPOINT_DIR = MODEL_ROOT / "Generated_Models" / "002002003009004_auc_preserving_calibrated_router_f2mi"

EXPERIMENT_ID = "002002003009004"
EXPERIMENT_LABEL = "AUC-preserving calibrated-router F2MI-Transformer"
...
def compute_losses(model, batch, cls_criterion, contour_bce, config: RunConfig, device):
    ...
    contour_floor_loss = F.relu(
        torch.as_tensor(config.contour_router_min, device=device) - router_weights[:, 1]
    ).pow(2).mean()
...
def run_epoch(model, loader, cls_criterion, contour_bce, optimizer, scaler, device, config: RunConfig):
    ...
    metrics["joint_score"] = float(
        config.monitor_auc_weight * metrics["auc"]
...
def evaluate_with_ids(model, loader, cls_criterion, contour_bce, device, config: RunConfig, split_name: str, outer_fold: int):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for idx, (row_id, sample_id, image_file, fold, label, prob) in enumerate(
                zip(batch["row_id"], batch["sample_id"], batch["image_file"], batch["fold"], labels, probs)
            ):
                records.append(
                    {
                        "row_id": int(row_id),
...
def train_fold(folds, outer_splits, masks_by_row_id, fold: int, args, config: RunConfig):
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        print(
            ...
            f"val_f1={val_metrics['f1']:.4f} val_joint={val_metrics['joint_score']:.4f} "
...
def write_summary(config: RunConfig, folds: pd.DataFrame, trained: bool, input_status=None, metrics=None) -> None:
    ...
    lines = [
        "# 002002003009004 AUC-Preserving Calibrated Router F2MI Run Summary",
        "",
        "## Status",
        "",
        ...
        "- Core mechanism: keep sample-adaptive multimodal routing, add weak ranking and contour-router preservation losses, and select checkpoints by a joint AUC/F1/recall score.",
        f"- Samples: {len(folds)}",
...
def write_training_progress(config: RunConfig, selected_folds: list[int], metrics_by_fold: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "## Fold Metrics",
        "",
        "| Fold | AUC | Acc | Pre | Recall | F1 | Best epoch |",
        "|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for _, row in metrics_by_fold.sort_values("fold").iterrows():
        lines.append(
            f"| {int(row['fold'])} | {row['auc']:.4f} | {row['acc']:.4f} | "
            f"{row['pre']:.4f} | {row['recall']:.4f} | {row['f1']:.4f} | {int(row['best_epoch'])} |"
        )
    lines += [
        "",
        "## Current Summary",
        ...
        json.dumps(
            {
                ...
                "rank_loss": float(loss_parts["rank_loss"].detach().cpu()),
                "contour_floor_loss": float(loss_parts["contour_floor_loss"].detach().cpu()),
...
def main() -> None:
    ...
    parser.add_argument("--lambda-rank", type=float, default=0.02)
    parser.add_argument("--lambda-contour-floor", type=float, default=0.001)
    ...
    parser.add_argument("--contour-router-min", type=float, default=0.10)
    parser.add_argument("--monitor", default="val_joint_score")
    parser.add_argument("--monitor-auc-weight", type=float, default=0.50)
    parser.add_argument("--monitor-f1-weight", type=float, default=0.30)
    parser.add_argument("--monitor-recall-weight", type=float, default=0.20)
    ...
    config = RunConfig(
        ...
        lambda_contour_floor=args.lambda_contour_floor,
        ...
        contour_router_min=args.contour_router_min,
        monitor_auc_weight=args.monitor_auc_weight,
        monitor_f1_weight=args.monitor_f1_weight,
        monitor_recall_weight=args.monitor_recall_weight,
