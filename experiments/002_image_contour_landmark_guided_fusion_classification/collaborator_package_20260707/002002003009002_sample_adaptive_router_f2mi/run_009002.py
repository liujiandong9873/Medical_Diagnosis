...
CHECKPOINT_DIR = MODEL_ROOT / "Generated_Models" / "002002003009002_sample_adaptive_router_f2mi"

EXPERIMENT_ID = "002002003009002"
EXPERIMENT_LABEL = "Sample-adaptive-router F2MI-Transformer"
...
def write_summary(config: RunConfig, folds: pd.DataFrame, trained: bool, input_status=None, metrics=None) -> None:
    ...
    lines = [
        "# 002002003009002 Sample-Adaptive-Router F2MI Run Summary",
        "",
        "## Status",
        "",
        ...
        "- Core mechanism: fractal tokens query image/contour/landmark/morphology tokens; the router uses per-sample multimodal summaries instead of fold-level fractal priors.",
        f"- Samples: {len(folds)}",
...
def main() -> None:
    ...
    parser.add_argument("--lambda-router-div", type=float, default=0.01)
