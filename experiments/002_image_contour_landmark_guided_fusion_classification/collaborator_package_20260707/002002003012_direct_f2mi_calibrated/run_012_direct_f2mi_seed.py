...
CHECKPOINT_ROOT = MODEL_ROOT / "Generated_Models" / "002002003012_direct_f2mi_calibrated_model"
CHECKPOINT_DIR = CHECKPOINT_ROOT / "seed_default"

EXPERIMENT_ID = "002002003012002"
EXPERIMENT_LABEL = "Seed robustness repeat for direct calibrated fractal-fractional multimodal interaction"

VARIANT_DESCRIPTIONS = {
    "direct_f2mi": "Direct F2MI without baseline-preserving residual fusion",
}
...
def variant_flags(variant: str) -> dict[str, bool]:
    ...
    return {
        "use_fractal_token": True,
        "use_fractional_operator": True,
...
def train_fold(folds, outer_splits, masks_by_row_id, fold: int, args, config: RunConfig):
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        monitor_value = row.get(config.monitor, val_metrics["f1"])
        ...
        print(
            ...
            f"test_acc={test_metrics['acc']:.4f} val_pre={val_metrics['pre']:.4f} "
            f"val_recall={val_metrics['recall']:.4f} val_f1={val_metrics['f1']:.4f} "
            f"baseline_f1={val_metrics['baseline_f1']:.4f} "
...
def write_summary(config: RunConfig, folds: pd.DataFrame, trained: bool, input_status=None, metrics=None) -> None:
    ...
    lines = [
        "# 002002003012002 Direct F2MI Seed Robustness Run Summary",
        "",
        "## Status",
        "",
        ...
        "- Core mechanism: initialize from the 002002002 contour-auxiliary baseline and learn a direct fractal/fractional multimodal interaction classifier.",
        "- Final decision: use the F2MI branch logit directly, followed by validation-fitted Platt calibration in analysis.",
        "- Purpose: test whether the direct calibrated F2MI candidate remains strong under an independent random seed.",
        f"- Samples: {len(folds)}",
...
def write_training_progress(config: RunConfig, selected_folds: list[int], metrics_by_fold: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "## Fold Metrics",
        "",
        "| Fold | Acc | Pre | Recall | F1 | Baseline F1 | Delta F1 | Residual | Best epoch |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for _, row in metrics_by_fold.sort_values("fold").iterrows():
        lines.append(
            f"| {int(row['fold'])} | {row['acc']:.4f} | {row['pre']:.4f} | "
...
def main() -> None:
    ...
    parser.add_argument("--variant", choices=sorted(VARIANT_DESCRIPTIONS), default="direct_f2mi")
    ...
    run_name = args.run_name if args.run_name else f"seed_{args.seed}"
    configure_variant_paths(run_name)
    flags = variant_flags(args.variant)

    config = RunConfig(
        experiment_id=f"{EXPERIMENT_ID}_{run_name}",
        experiment_label=f"{EXPERIMENT_LABEL}: {VARIANT_DESCRIPTIONS[args.variant]} ({run_name})",
