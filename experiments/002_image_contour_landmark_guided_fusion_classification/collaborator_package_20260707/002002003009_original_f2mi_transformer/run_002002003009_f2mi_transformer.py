...
CHECKPOINT_DIR = MODEL_ROOT / "Generated_Models" / "002002003009_fractal_guided_multimodal_interaction"

EXPERIMENT_ID = "002002003009"
EXPERIMENT_LABEL = "F2MI-Transformer fractal-guided multimodal interaction"
...
class FractalGuidedInteraction(nn.Module):
    ...
    def forward(
        ...
        router_logits = self.router(fractal_pooled)
...
class F2MITransformer(nn.Module):
    def __init__(
        ...
        self.interaction = FractalGuidedInteraction(config.d_model, config.n_heads, config.dropout)
...
def write_summary(config: RunConfig, folds: pd.DataFrame, trained: bool, input_status=None, metrics=None) -> None:
    ...
    lines = [
        "# 002002003009 F2MI-Transformer Run Summary",
        "",
        "## Status",
        "",
        ...
        "- Core mechanism: fractal tokens act as multimodal queries, router variables, and fractional alpha controller.",
        f"- Samples: {len(folds)}",
    ...
    if trained and metrics is not None and not metrics.empty:
        ...
        for metric in ["auc", "acc", "pre", "recall", "f1", "router_image", "router_contour", "router_landmark", "router_morphology"]:
